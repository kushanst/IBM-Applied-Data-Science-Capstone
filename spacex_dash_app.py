...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                max=10000, step=1000, value=[min_payload, max_payload]),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        temp = spacex_df['Launch Site']==entered_site
        filtered_df = spacex_df[temp]
        filtered_df=filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig = px.pie(filtered_df, values = 'class count', names = 'class', 
        title = 'Success and failure of ' + entered_site)
        return fig
